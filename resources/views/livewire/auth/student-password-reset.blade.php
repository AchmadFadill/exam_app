<div class="space-y-5" @if($isPending) wire:poll.3s="checkStatus" @endif>
    @if ($successMessage)
        ...
        @if(!$isPending)
            <x-button href="{{ route('student.login') }}" variant="primary" class="w-full py-3.5 px-6 !rounded-2xl font-bold uppercase tracking-widest text-sm shadow-xl shadow-blue-900/20 active:scale-[0.98]">
                LOGIN SEKARANG
            </x-button>
        @endif
    @else
