<x-app-layout :title="$title ?? 'Siswa'">
    <!-- Sidebar -->
    <x-slot name="sidebar">
        <x-sidebar>
            <div class="px-3 text-xs font-semibold text-blue-300 uppercase tracking-wider mb-2">Menu Utama</div>
            
            <x-sidebar-link :href="route('student.dashboard')" :active="request()->routeIs('student.dashboard')">
                <x-slot name="icon">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path>
                </x-slot>
                Beranda
            </x-sidebar-link>



            <div class="px-3 text-xs font-semibold text-blue-300 uppercase tracking-wider mt-6 mb-2">Ujian</div>
            
             <x-sidebar-link :href="route('student.results')" :active="request()->routeIs('student.results*')">
                <x-slot name="icon">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </x-slot>
                Hasil Ujian
            </x-sidebar-link>

            <div class="px-3 text-xs font-semibold text-blue-300 uppercase tracking-wider mt-6 mb-2">Sistem</div>
            <x-sidebar-link :href="route('student.settings')" :active="request()->routeIs('student.settings')">
                <x-slot name="icon">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                </x-slot>
                Pengaturan
            </x-sidebar-link>

        </x-sidebar>
    </x-slot>

    <!-- Top Navbar -->
    <x-slot name="navbar">
        <x-navbar :title="View::yieldContent('title', 'Beranda')" userPrefix="Siswa" />
    </x-slot>

    <!-- Content -->
    {{ $slot }}
</x-app-layout>
